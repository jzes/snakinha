...
(t/deftest bear-fruit-test
  (t/testing "should return fruit coordenates"
    (let [{:keys [input-board
                  input-snake
                  input-xy
                  expected]} bear-fruit-success
          {:keys [x y]} input-xy]
      (t/is (= expected (fruit/bear-fruit x y input-board input-snake)))))
  (t/testing "should throw a exception out of bounds"
    (let [{:keys [input-board
                  input-snake
                  input-xy
                  expected]} bear-fruit-error-out
          {:keys [x y]} input-xy]
      (t/is (thrown-with-msg? java.lang.Exception expected (fruit/bear-fruit x y input-board input-snake)))))
  (t/testing "should throw a exception cant bear over snake"
    (let [{:keys [input-board
                  input-snake
                  input-xy
...
(t/deftest draw-fruit-test
  (t/testing "should draw a fruit in a board correctly"
    (let [{:keys [input expected]} draw-fruit-succes
          {:keys [board xy sprite]} input]
      (t/is (= expected (fruit/draw-fruit board xy sprite))))))